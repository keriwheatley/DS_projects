...
Self-Reported Response: *Last month, was your total household income before taxes more or less than (amount) per month?* (Sample Count: *`r count_erincome`*)
...
Self-Reported Response: *In the past 30 days, did you or any member of this household receive SNAP or food stamp benefits?* (Sample Count: *`r count_eusnap`*)
...
Self-Reported Response: *In the last 30 days, did you or any member of your household receive benefits from the WIC program, that is, the Women, Infants, and Children program?* (Sample Count: *`r count_euwic`*)
...
Self-Reported Response: *Change in spouse or unmarried partner's labor force status or full time or part time employment status between CPS and ATUS* (Sample Count: *`r count_erspemch`*)
...
### Weight
```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(euwgt),y=jitter(erbmi))) + geom_point(size = .10, colour="orange") + geom_smooth() + xlab("Weight") + ylab("BMI")
```
...
### Height
```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(euhgt),y=jitter(erbmi))) + geom_point(size = .10, colour="orange")+ geom_smooth() + xlab("Height") + ylab("BMI")
```
...
ggplot(resp[erbmi>0 & euexfreq>0],aes(x = jitter(erbmi),y=jitter(euexfreq))) + geom_point(size = .10, colour="green")+ geom_smooth() + xlab("BMI") + ylab("Frequency of Exercise in Past Week")
```
...
```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0 & eufastfdfrq>0],aes(x = jitter(eufastfdfrq),y=jitter(erbmi))) + geom_point(size = .5, colour="orange") + geom_smooth() + xlab("Fast Food Amount") + ylab("BMI")
```
...
### Primary Eating
```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(erbmi),y=jitter(ertpreat)))+geom_point(size = .1, colour="green")+geom_smooth()+ xlab("BMI") + ylab("Primary Eating (in minutes)")
```
...
### Secondary Eating
```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(erbmi),y=jitter(ertseat)))+geom_point(size = .1, colour="green")+geom_smooth()+ xlab("BMI") + ylab("Secondary Eating (in minutes)")
```

Log transformed variable

```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(erbmi),y=jitter(log(ertseat+1))))+geom_point(size = .1, colour="green")+geom_smooth()+ xlab("BMI") + ylab("Secondary Eating (in minutes)")
```

```{r, echo=FALSE, fig.width=5, fig.height=4}
ggplot(resp[erbmi>0],aes(x = jitter(log(ertseat+1)),y=jitter(erbmi)))+geom_point(size = .1, colour="green")+geom_smooth()+ xlab("Secondary Eating (in minutes)") + ylab("BMI")
```
...
model1 = lm(BMI ~ Exercise_Freq, data = linreg)
model2 = lm(BMI ~ Exercise_Freq + SNAP, data = linreg)
model3 = lm(BMI ~ Excellent_Health + Very_Good_Health + Good_Health + Fair_Health, data = linreg)
model4 = lm(BMI ~ Exercise_Freq+ Excellent_Health + Very_Good_Health + Good_Health + Fair_Health, data = linreg)
model5 = lm(BMI ~ Exercise_Freq + Fast_Food_Freq, data = linreg)
model6 = lm(BMI ~ Exercise_Freq + SNAP + Excellent_Health + Very_Good_Health + Good_Health + Fair_Health +Fast_Food_Freq, data = linreg)
model6 = lm(BMI ~ Exercise_Freq + Health, data = linreg)
model7 = lm(BMI ~ Exercise_Freq + Health + SNAP, data = linreg)
...
robust_se.model6 = sqrt(diag(vcovHC(model6)))
robust_se.model7 = sqrt(diag(vcovHC(model7)))
```
...
omit.stat = "f",
se = list(robust_se.model1, robust_se.model2, robust_se.model3, robust_se.model4, robust_se.model5, robust_se.model6, robust_se.model7),
...
AIC6 = signif(AIC(model6),4)
AIC7 = signif(AIC(model7),4)
```
...
```{r include=FALSE}
modela = lm(Health ~ Exercise_Freq, data = linreg)
modelb = lm(Health ~ BMI, data = linreg)
modelc = lm(Health ~ Fast_Food_Freq, data = linreg)
modeld = lm(Health ~ Exercise_Freq + BMI + Fast_Food_Freq, data = linreg)
